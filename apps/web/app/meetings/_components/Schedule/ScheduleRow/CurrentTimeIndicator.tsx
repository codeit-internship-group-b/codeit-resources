...
export default function CurrentTimeIndicator(props: CurrentTimeIndicatorProps): JSX.Element {
  ...
  return (
    <div>
      <div
        className="border-custom-black md:top-30 absolute top-0 z-0 h-full border-l-2"
        style={{ left: `${String(currentPosition)}px` }}
      />

      <div
        className="text-xs-semibold text-custom-black absolute -bottom-24 z-0 -ml-16 !hidden rounded bg-none md:-bottom-20 md:block"
        style={{ left: `${String(currentPosition)}px` }}
      >
        {currentTime}
      </div>
